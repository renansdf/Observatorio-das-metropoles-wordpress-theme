...
<style>
	...
	.biblioteca-search-form{
			max-width: 500px;width: 90%;margin: 0 auto 50px;	}

	.biblioteca-search-form form{
			display: flex;	flex-direction: column;	align-items: center;	}

	.biblioteca-search-form input[type="text"]{
	...
	.search-tooltip{
			text-align: center;	width: auto;	display: flex;
			align-items: center;	justify-content: center;	margin: 10px auto;	}

	.search-tooltip div{
			color: #fff;	background-color: grey;	width: 20px;	height: 20px;
			border-radius: 50%;	font-weight: bold;	position: relative;	}

	.search-tooltip div span{
			display: none; position: absolute; width: 220px;
			z-index: 9; top: -15px; background-color: grey; color: #fff;
			border-radius: 3px; left: -102px; padding: 7px 10px; 
			font-size: 11px; line-height: 1.1em; font-weight: normal;
	}

	.search-tooltip div:hover span{
			display: block;	}

	.biblioteca-search-form input[type="text"]::placeholder{
			color: grey;	}

	.biblioteca-search-form input[type="submit"]{
			background-color: grey!important;		padding: 10px 15px!important;
...
<div id="page-header-bg" data-midnight="light" data-text-effect="none" data-bg-pos="center" data-alignment="center" data-alignment-v="middle" data-height="250">
	...
	<div class="container search-header-text">
		<div class="row">
			<div class="col span_6 ">
				<div class="inner-wrap">
					<h1 class="color-dark-grey"><?php echo esc_html__( 'Results For', 'salient' ); ?> <span>"<?php echo esc_html( get_search_query( false ) ); ?>"</span></h1>	
					<?php
					if ( $wp_query->found_posts ) {
						echo '<span class="result-num color-dark-grey">' . esc_html( $wp_query->found_posts ) . ' ' . esc_html__( 'results found', 'salient' ) . '</span>'; }
					?>
				</div>
			</div>
		</div>
	...
	<div class="container biblioteca-search-form">
		<form action="/" method="get">
			<input type="text" name="s" id="search" value="" placeholder="<?= $_GET['cat'] ? 'Pesquise nesta categoria' : 'Pesquise em todo nosso acervo';?>" />

			<div class="search-tooltip">
				<div>?<span>coloque os termos de pesquisa entre aspas para procurar por uma frase. 
				</span></div>
			</div>
...
<div class="container-wrap" data-layout="<?php echo esc_attr( $search_results_layout ); ?>">
	...
	<div class="container main-content">
		...
		<div class="row">
							...
							if ( get_post_type( $post->ID ) === 'post' ) {
								...
								<article class="result" data-post-thumb="<?php echo esc_attr( $using_post_thumb ); ?>">
									<div class="inner-wrap">
										...
										?>
										<h2 class="title"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a> <span>
                    <?php
                      $categories = get_the_category($post->ID);
                      foreach($categories as $cat_OBJ){
                        $cat_url = get_category_link($cat_OBJ->cat_ID);
                        echo '<a href="'. $cat_url .'">'. $cat_OBJ->name . '</a> ';
                      }
                    ?>
                    </span></h2>
            ...
            <div class="prev"><?php previous_posts_link( '&laquo; resultados anteriores' ); ?></div>
						<div class="next"><?php next_posts_link( 'próximos resultados &raquo;', '' ); ?></div>
					</div>	
				<?php } ?>
